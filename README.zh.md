...
| 版本 | 重点 |
|---|---|
| **v2.54.0** | 🧹 **Housekeeping + 264 测试 + 性能回归套件**。备份保留策略 (`api_backup_prune` keepLast/retentionDays + 防 0 备份安全网)，AR 过期条目清理 (`api_auto_resume_prune_stale` 仅清理终态 + 30 天以上)，新增 `server/housekeeping.py` 编排器 + 磁盘使用报告。UI "🧹 清理" 卡片：dry-run 预览 → 确认 → 真实执行流程。测试数 171 → 264 (+93) — 覆盖 backup/learner/hyper_agent/briefing/system + 新增性能回归套件（17 个计时预算断言）。 |
| **v2.53.0** | 💾 **备份/恢复 + 🔍 会话搜索 + 171 测试**。新增 `server/backup.py`：所有 `~/.claude-dashboard-*.json` + SQLite (`VACUUM INTO`) 快照到 `~/.claude-dashboard-backups/lazyclaude-<ts>.tar.gz`，atomic `.tmp+rename` + manifest。`reliability` 分组新增 `💾 备份 & 恢复` 标签暴露 list/create/restore/delete — 含路径包容性检查、覆盖确认、manifest 签名守卫。新增 `/api/sessions/search?q=&limit=&cwd=` 按行流式扫描 JSONL（不全量加载），按出现次数 + 新鲜度评分，top-200 会话上限，每会话 ≤5 匹配早终止，30 秒 TTL 缓存。会话标签新增 debounced 搜索框 + 结果表。测试数 113 → 171 (+58) — 覆盖 hooks/mcp/cost_timeline/notify。 |
| **v2.52.0** | 🧠 **Hyper-Advisor + 113 测试 + 467× AR status**。承接 v2.49.0 推迟的 Hyper-Agent ↔ Auto-Resume 集成：新 `hyper_advise_auto_resume(entry, recent_failures)` 按各 exit reason 的重试策略决策规则调用 Haiku 元-LLM 并返回 clamped JSON 提议。UI 每行新增 "🧠 Hyper Advisor" 按钮 → 弹窗显示当前值 vs 建议 pollInterval / maxAttempts / promptHint / rationale → "应用" 合并到现有 entry。测试数 68 → 113 (+45) — 覆盖 workflows.py、ai_providers.py、ccr_setup.py。`/api/auto_resume/status` 327 ms → 0.7 ms（467×）— 0 绑定时短路，跳过 lsof+ps 交叉引用。 |
| **v2.51.0** | 🛠️ **UX 强化 — QS 卡顿修复 + 吉祥物开关 + 当前参数 + AR 终端绑定 + 🛟 可靠性分组**。Quick Settings 标签点击双重渲染（innerHTML + 递归 openQuickSettings）→ 提取 `_qsRefreshSection`，每次点击 `_qsRenderShell` ×1、`_qsBindControls` ×1。吉祥物定时器当隐藏时 early-exit。Quick Settings 第 5 个新分区 `🔎 当前参数`（只读）：有效 prefs + 运行时信息 + 端点快捷链接。Auto-Resume 绑定仅限**当前运行中的 CLI 会话** — `api_auto_resume_set` 在没有活动 PID 时拒绝（或通过 `allowUnboundSession=true` 绕过）。每个绑定附带 `pid`、`terminal_app`、`liveSession`、`terminalClosedAction`（`wait`/`cancel`/`exhaust`）字段。`cancel` 在 3 个 dead ticks 后自动停止。UI 新增终端列 + 🟢/⚪ 活动芯片。Auto-resume 标签从 `observe` 移至新的 `🛟 可靠性` 分组。 |
| **v2.50.0** | 📊 **可观测性 + 可靠性 — telemetry、成本建议、+41 测试**。将 v2.46.0–v2.49.0 静默累积的数据呈现出来。工作流标签新增 `📊 执行遥测` 面板：基于 v2.47.0 `workflow_runs` SQLite 的 per-workflow p50/p95/p99 + 成功/重试率 + 费用，可选窗口 (1h/24h/7d/30d)，30 秒自动刷新且 visibility 守卫。费用时间线标签新增 `💡 成本优化建议` 面板：4 类规则建议（短提示词 → Haiku 切换、长上下文 → 提示词缓存、重复批处理 → 本地 ollama、旧模型 → 升级）+ 预估节省。测试数 27 → 68 (+41) — 覆盖 `db.py / prefs.py / process_monitor.py`。新增端点：`/api/workflows/telemetry`、`/api/costs/recommendations`。 |
| **v2.49.0** | 🔄 **Auto-Resume 强化** — `observe` 分组新增 `🔄 Auto-Resume 管理` 标签（活动绑定表 + 批量取消 + 状态计数芯片 + 10 秒自动刷新带 visibility 守卫）。通知渠道扩展：SMTP+STARTTLS 邮件、Telegram Bot API（与现有 Slack/Discord 并列）。`_sanitize_notify` + `_send_notify` 已 wiring。Haiku summary 旁路：新 `scripts/ar-haiku-summary.py`（仅 stdlib 198 行，直接调用 Anthropic Messages API，6 种独立退出码，`--dry-run` 隐去密钥）。通过 `install(use_direct_api=True)` 启用（默认 false，向后兼容）。`auto_resume_hooks.py` 添加 36 行 docstring 说明 snapshot+inject 机制和两种 Haiku 后端。**项目首个 pytest harness**：`tests/test_auto_resume.py` 26 个用例（`_classify_exit / _parse_reset_time / _exponential_backoff / _push_hash_and_check_stall / _jsonl_idle_seconds`）。`make test` 目标。 |
| **v2.48.1** | 🔄 **Auto-Resume worker 并发** — 单线程串行 retry 循环替换为 `ThreadPoolExecutor(max_workers=4)` 每 tick 扇出。N 个待处理会话原本需 N×retry-time 串行等待，现可并发处理 4 个。锁规则保持（`_process_one` 持 `_LOCK` 用于 JSON IO，`_RUNNING_PROCS` 阻止同 sid 重入）。worker 关闭时 `cancel_futures=True` 清理 pool。|
| **v2.48.0** | 🧹 **第三阶段 — 死代码清理 + EXPLAIN 驱动索引 + CSS 精简**。JS：移除孤立的 `VIEWS.design` + `addDesignDir`（128 行，无 NAV 入口）、未使用的 `_wfAddNode`、`_wfInspectorBody`（已被替代）、`_wfNodeSet`（旧变体）— 共 **354 行 JS / -23 KB**。Python：`system.py / auth.py / toolkits.py` 移除 4 个未使用 import。CSS：3 个 dead 自定义类（`.card-hi`、`.divider`、`.group-label`）；主题/状态类验证为动态设置后保留。数据库：对 `server/*.py` 中每个静态 SQL 执行 `EXPLAIN QUERY PLAN`；**新增 7 个索引** 将 `SCAN + TEMP B-TREE` 转为 `SCAN/SEARCH USING INDEX`。`_db_init` 中执行一次 `ANALYZE`（4.8 ms）。DB 索引数 12 → 19。 |
| **v2.47.0** | 🚀 **第二阶段性能 — workflows.runs → SQLite + RSS 缩小 27× + 前端整合**。运行状态从单一 JSON blob 迁移到 `workflow_runs` SQLite 表（含 `idx_runs_workflow / idx_runs_status` 索引）。`_LOCK` 现仅覆盖定义部分 — 并发 save 不再因整文件 fsync 而串行化。一次性迁移以标志记录在 JSON store 中，旧 `runs` dict 保留以便回滚。tracemalloc 剖析：`_index_jsonl` 原本将整个 JSONL 读入内存 + 三次独立遍历，重写为单遍流式扫描（稳态 **700 MB → 42 MB**，强制重建索引 **1947 MB → 102 MB**）。在线服务现 57 MB RSS（原 1577 MB，约 27×）。前端：会话表 IntersectionObserver 虚拟滚动（每 50 行一批），8 处 Chart.js destroy+new 改为 `chart.update('none')`，9 个 global keydown 监听器合并为单一调度器（额外修复了 `_wfBindCanvas` 每次重新绑定的潜在泄漏），`_makeDraggable` 的 document 监听器泄漏通过 `_detachDragListeners` 修复。 |
| **v2.46.0** | 🚀 **综合性能扫描 — 33 项手术式优化**（后端 12 + 前端 8 + 启动/静态/i18n 4 + 推迟至 v2.47.0+ 共 9 项）。后端：`_db_init` 加锁守卫（曾每次请求执行），新增 3 个缺失的 SQL 索引，hooks/translations/ollama-models TTL 缓存，`lsof` TCP/UDP 并行化，sessions N+1 SQL bug 修复（游标越作用域），learner JSONL 遍历改为索引 SQL，MCP 模块级磁盘 I/O 延迟。前端：`_wfUpdateNodeTransform` 60fps `querySelector` → Map 查表，viewport ref 缓存，`escapeHtml` map 提升至模块级，11 个只读端点改用 `cachedApi`，5 个轮询定时器加 `document.hidden` 守卫，`_apiCache` LRU 上限，移除重复 resize 监听器，`_recentTabsCache` 记忆化。启动：`background_index` + `_auto_start_ollama` 守护线程化 — `Serving http://...` 现在先于 I/O 打印。静态：`ETag` + `If-None-Match` → 304 短路，locale JSON 通过缓存 gzip（曾每次请求未压缩），`_STATIC_CACHE` LRU 上限。i18n：流水线 mtime 守卫（无源变更则跳过）。实测：`/api/hooks/recent-blocks` 冷→热 **2754 ms → 4 ms（~700×）**，`_db_init` 第二次调用 0.00 ms。 |
| **v2.45.2** | 🐛 **修复 + 🔌 自动启动开关** — Ollama 标签的"已安装模型"表格永远为空：`_ollamaLoadInstalled` 填充了数据但从未调用 `_ollamaRenderInstalled()`。现已修复，每行的 🗑 删除 + 详情按钮终于显示。**新增 pref `behavior.autoStartOllama`**（默认 `true`，向后兼容），在 Quick Settings (⌘,) 中切换。关闭后启动时跳过 `ollama serve` 自动生成，节省 idle RSS。 |
