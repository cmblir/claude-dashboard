...
6 组 49 个标签页 · 16 种工作流节点 · 8 个 AI 供应商 · 5 种主题 · 3 种语言。
...
|---|---|
| `cat ~/.claude/settings.json` 肉眼检查 | 49 个标签页各自渲染对应切片 |
...
### 🤝 Claude Code 集成（49 个标签页）

| 分组 | 标签页 |
|---|---|
| 🆕 新功能 | `features` · `onboarding` · `guideHub` · 🆕 `claudeDocs` |
| 🏠 主要 | `overview` · `projects` · `analytics` · `aiEval` · `sessions` |
| 🛠️ 工作 | `workflows` · `aiProviders` · `agents` · `projectAgents` · `skills` · `commands` · `promptCache` · `thinkingLab` · `toolUseLab` · `batchJobs` · `apiFiles` · `visionLab` · `modelBench` · 🆕 `serverTools` · 🆕 `citationsLab` · 🆕 `agentSdkScaffold` |
...
- **每种语言 3,157 个翻译键** · **英文/中文模式下韩文残留为 0**（已验证）
...
│   ├── nav_catalog.py            # 49 个标签页单一数据源 + i18n 描述
...
│   └── locales/{ko,en,zh}.json   # 3,157 键 × 3 语言
├── tools/
...
| 指标 | 值 |
|---|---|
| 后端代码 | ~17,000 行 · 42 个模块 · 仅标准库 |
| 前端代码 | ~15,500 行 · 单 HTML 文件 |
| API 路由 | **178**（GET 97 / POST 78 / PUT 3 + regex webhook） |
| 标签页 | **49**（6 组） |
| 工作流节点类型 | **16** |
| AI 供应商 | **8** 个内置 + 无限自定义 |
| Claude API 实验室标签 | **10**（提示缓存 · Extended Thinking · Tool Use · Batch · Files · Vision · 模型基准 · Server Tools · Citations · Agent SDK 脚手架） |
| 官方文档索引 | **33** 页（Claude Code + API + SDK + Models + Account） |
| Ollama 目录 | **23** 个模型 |
| 子代理角色预设 | **16** |
| 内置工作流模板 | **8**（内置 5 + 团队 3） |
| i18n 键 | **3,157** × 3 语言 · 缺失 0 |
| 主题 | **5** |
| 教程场景 | **18** |
...
```bash
npm run test:e2e:smoke       # 49 个标签页 — 渲染失败 / console error 检测
