...
6 组 51 个标签页 · 16 种工作流节点 · 8 个 AI 供应商 · 5 种主题 · 3 种语言。
...
|---|---|
| `cat ~/.claude/settings.json` 肉眼检查 | 51 个标签页各自渲染对应切片 |
...
- **每种语言 3,212 个翻译键** · **英文/中文模式下韩文残留为 0**（已验证）
...
│   ├── nav_catalog.py            # 51 个标签页单一数据源 + i18n 描述
...
│   └── locales/{ko,en,zh}.json   # 3,212 键 × 3 语言
├── tools/
...
| 指标 | 值 |
|---|---|
| 后端代码 | ~17,600 行 · 44 个模块 · 仅标准库 |
| 前端代码 | ~16,300 行 · 单 HTML 文件 |
| API 路由 | **188**（GET 101 / POST 84 / PUT 3 + regex webhook） |
| 标签页 | **51**（6 组） |
| 工作流节点类型 | **16** |
| AI 供应商 | **8** 个内置 + 无限自定义 |
...
```bash
npm run test:e2e:smoke       # 51 个标签页 — 渲染失败 / console error 检测
