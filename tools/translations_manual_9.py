...
NEW_EN = {
    ...
    # ── v2.10.1 노드 편집 모달 카테고리 토글 ────────────────────
    "타입": "Type",
    "타입 변경": "Change type",
    "접기": "Collapse",
    "펼치기": "Expand",

    # ── v2.10.2 노드 hover 결과 tooltip ─────────────────────────
    "건너뜀": "Skipped",
    "제공자": "Provider",
    "토큰": "Tokens",

    # ── v2.10.4 team 탭 — t() shadowing 버그 수정 부수 i18n ──
    "이름": "Name",
    "이메일": "Email",
    "계정 UUID": "Account UUID",
    "플랜": "Plan",
    "추가 사용 허용": "Extra usage",
    "조직 이름": "Org name",
    "조직 UUID": "Org UUID",
    "조직 역할": "Org role",
    "워크스페이스 역할": "Workspace role",
    "참고": "Note",
    ...
    # ── v2.16.0 Prompt Library ────────────────────────────────
    "프롬프트 라이브러리": "Prompt Library",
    "자주 쓰는 프롬프트 저장 · 태그 검색 · 워크플로우로 복제": "Save prompts · search by tag · convert to workflow",
    "자주 쓰는 프롬프트를 태그와 함께 저장하고, 복사/복제/워크플로우 복제 버튼으로 바로 재활용합니다.":
        "Save prompts with tags and reuse via copy / duplicate / convert-to-workflow actions.",
    "제목": "Title",
    "본문": "Body",
    "태그 (쉼표 구분)": "Tags (comma-separated)",
    "모델 미지정": "No model",
    "새 프롬프트": "New prompt",
    "수정": "Edit",
    "복제": "Duplicate",
    ...
    "복제 완료": "Duplicated",
    "복제 실패": "Duplicate failed",
    "워크플로우 생성": "Workflow created",
    "제목을 입력하세요": "Enter a title",
    "본문을 입력하세요": "Enter body",
    "저장 완료": "Saved",
    ...
    # ── Batch Jobs (v2.5.0) ────────────────────────────────────
    "배치 작업": "Batch Jobs",
    "Message Batches API 로 여러 프롬프트를 병렬 제출하고 상태 · JSONL 결과를 확인.":
        "Submit many prompts in parallel via Message Batches API and track status + JSONL results.",
    "Q&A 10건": "Q&A (10)",
    "요약 5건": "Summarize (5)",
    "프롬프트 (한 줄에 하나)": "Prompts (one per line)",
    "예: 한국의 수도는?": "e.g., What is the capital of Korea?",
    "최대 1000건": "Up to 1000 prompts",
    "배치 제출": "Submit batch",
    "새로고침": "Refresh",
    "배치 제출 확인": "Confirm batch submission",
    "총 {n} 건을 {model} 로 제출합니다. Anthropic API 요금이 발생할 수 있습니다. 계속할까요?":
        "Submitting {n} prompts with {model}. Anthropic API charges may apply. Continue?",
    "제출 중…": "Submitting…",
    "배치 제출 완료": "Batch submitted",
    "제출 실패": "Submission failed",
    "프롬프트를 1건 이상 입력하세요": "Enter at least one prompt",
    "최근 배치": "Recent batches",
    "결과 미리보기": "Results preview",
    "결과 보기": "View results",
    "배치 없음": "No batches",
    "배치를 선택해서 결과를 보세요": "Select a batch to view results",
    "결과": "Results",
    "건": "items",
    "결과 조회 실패": "Failed to load results",
    "완료 전에는 결과가 비어있을 수 있습니다": "Results may be empty before completion",
    "배치 취소": "Cancel batch",
    ...
    # v2.24.0 — RTK Optimizer
    "rtk_subtitle": "A Rust CLI proxy that cuts Claude token usage by 60-90% (rtk-ai/rtk) — install, activate, and inspect savings in one tab.",
    "rtk_installed": "Installed",
    "rtk_not_installed": "Not installed",
    "rtk_hook_active": "Hook active",
    "rtk_hook_inactive": "Hook inactive",
    "rtk_config_label": "Config file",
    "rtk_view_config": "View content",
    "rtk_links_label": "Links",
    "rtk_guide": "Official guide",
    "rtk_install_heading": "Install",
    "rtk_install_desc": "Pick a method — a Terminal window opens and runs the real installer.",
    "rtk_no_brew": "Homebrew not installed — use curl or cargo instead.",
    "rtk_hook_heading": "Claude Code hook",
    "rtk_hook_desc": "Running rtk init -g wraps Claude Code's Bash commands (e.g. git status) so the LLM sees compressed output.",
    "rtk_run_init": "Run rtk init -g",
    "rtk_hook_detected": "Already detected",
    "rtk_gain_heading": "Cumulative token savings (rtk gain)",
    "rtk_session_heading": "Current session (rtk session)",
    "rtk_commands_heading": "Command reference",
    "rtk_commands_desc": "rtk is an output-compression proxy for common dev commands. E.g. git status → rtk git status.",
    "rtk_ultra_hint": "💡 Use -u / --ultra-compact for ASCII icons + extra savings.",
    "rtk_install_confirm": "This opens a Terminal and runs the installer. Continue?",
...
NEW_ZH = {
    ...
    # ── v2.10.2 节点悬停结果 tooltip ───────────────────────────
    "건너뜀": "已跳过",
    "제공자": "提供者",
    "토큰": "令牌",

    # ── v2.10.4 team 标签页 i18n ───────────────────────────────
    "이름": "姓名",
    "이메일": "邮箱",
    "계정 UUID": "账户 UUID",
    "플랜": "套餐",
    ...
    # ── v2.16.0 提示库 ────────────────────────────────────────
    "프롬프트 라이브러리": "提示库",
    "자주 쓰는 프롬프트 저장 · 태그 검색 · 워크플로우로 복제": "保存常用提示 · 按标签搜索 · 转换为工作流",
    "자주 쓰는 프롬프트를 태그와 함께 저장하고, 복사/복제/워크플로우 복제 버튼으로 바로 재활용합니다.":
        "用标签保存常用提示,通过复制/复制/转换为工作流按钮直接复用。",
    "제목": "标题",
    "본문": "正文",
    "태그 (쉼표 구분)": "标签（逗号分隔）",
    "모델 미지정": "未指定模型",
    ...
    "배치 작업": "批量任务",
    "Message Batches API 로 여러 프롬프트를 병렬 제출하고 상태 · JSONL 결과를 확인.":
        "通过 Message Batches API 并行提交多个提示,查看状态和 JSONL 结果。",
    "Q&A 10건": "Q&A 10 条",
    "요약 5건": "摘要 5 条",
    "프롬프트 (한 줄에 하나)": "提示（每行一条）",
    "예: 한국의 수도는?": "例如：韩国的首都是?",
    "최대 1000건": "最多 1000 条",
    "배치 제출": "提交批次",
    "새로고침": "刷新",
    "배치 제출 확인": "确认批次提交",
    "총 {n} 건을 {model} 로 제출합니다. Anthropic API 요금이 발생할 수 있습니다. 계속할까요?":
        "将使用 {model} 提交 {n} 条提示。可能产生 Anthropic API 费用。继续吗?",
    "제출 중…": "提交中…",
    "배치 제출 완료": "批次已提交",
    "제출 실패": "提交失败",
    "프롬프트를 1건 이상 입력하세요": "请至少输入一条提示",
    "최근 배치": "最近批次",
    "결과 미리보기": "结果预览",
    "결과 보기": "查看结果",
    "배치 없음": "无批次",
    "배치를 선택해서 결과를 보세요": "选择批次查看结果",
    "결과": "结果",
    "건": "条",
    "결과 조회 실패": "无法加载结果",
    "완료 전에는 결과가 비어있을 수 있습니다": "完成前结果可能为空",
    "배치 취소": "取消批次",
    "이 배치를 취소할까요?": "取消此批次?",
    "취소 요청됨": "已请求取消",
    "취소 실패": "取消失败",
    ...
    # v2.24.0 — RTK Optimizer
    "rtk_subtitle": "将 Claude token 消耗减少 60-90% 的 Rust CLI 代理 (rtk-ai/rtk) — 在一个标签页中安装、激活、查看节省。",
    "rtk_installed": "已安装",
    "rtk_not_installed": "未安装",
    "rtk_hook_active": "钩子已启用",
    "rtk_hook_inactive": "钩子未启用",
    "rtk_config_label": "配置文件",
    "rtk_view_config": "查看内容",
    "rtk_links_label": "链接",
    "rtk_guide": "官方指南",
    "rtk_install_heading": "安装",
    "rtk_install_desc": "选择一种方法 — 将打开 Terminal 窗口并运行真实安装程序。",
    "rtk_no_brew": "未安装 Homebrew — 请使用 curl 或 cargo。",
    "rtk_hook_heading": "Claude Code 钩子",
    "rtk_hook_desc": "运行 rtk init -g 会自动包装 Claude Code 的 Bash 命令（如 git status），让 LLM 看到压缩后的输出。",
    "rtk_run_init": "运行 rtk init -g",
    "rtk_hook_detected": "已检测到",
    "rtk_gain_heading": "累计 token 节省 (rtk gain)",
    "rtk_session_heading": "当前会话 (rtk session)",
    "rtk_commands_heading": "命令参考",
    "rtk_commands_desc": "rtk 是常见开发命令的输出压缩代理。例如：git status → rtk git status。",
    "rtk_ultra_hint": "💡 使用 -u / --ultra-compact 可启用 ASCII 图标并进一步节省。",
    "rtk_install_confirm": "将打开 Terminal 并运行安装程序。继续吗？",
