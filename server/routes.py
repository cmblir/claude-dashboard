...
from .actions import (
    api_chat, api_lazyclaw_chat, api_lazyclaw_chat_ping, api_lazyclaw_term,
    api_session_spawn, handle_chat_stream,
...
from .orchestrator import (
    ...
    api_orch_start, api_orch_sweeper_status, api_orch_unbind,
...
from .workflows import (
    ...
    api_workflow_preflight,
    api_workflow_run_node,
    api_workflow_run_cancel,
    api_workflow_run_force_finish,
...
ROUTES_GET: dict[str, Callable[[dict], Any]] = {
    ...
    "/api/orchestrator/sweeper-status": api_orch_sweeper_status,
...
ROUTES_POST: dict[str, Callable[[dict], Any]] = {
    ...
    "/api/lazyclaw/chat/ping": api_lazyclaw_chat_ping,
    "/api/lazyclaw/term": api_lazyclaw_term,
    ...
    "/api/workflows/run-cancel": api_workflow_run_cancel,
    "/api/workflows/run-force-finish": api_workflow_run_force_finish,
    "/api/workflows/preflight": api_workflow_preflight,
    "/api/workflows/run-node": api_workflow_run_node,
    ...
    "/api/slack/events":       api_slack_events,
    "/api/telegram/config/save":  api_telegram_config_save,
    "/api/telegram/config/clear": api_telegram_config_clear,
    "/api/telegram/test":         api_telegram_test,
    "/api/telegram/webhook":      api_telegram_webhook,
    "/api/orchestrator/config/save": api_orch_config_save,
    "/api/orchestrator/bind":        api_orch_bind,
    ...
    def _send_static(self, path: str) -> None:
        ...
        if not entry or entry[0] != cache_mtime:
            try:
                ...
                self.send_response(500); self.end_headers(); return
            # KK2 (v2.66.29) — when serving index.html, rewrite the
            # `<script src="/app.js">` reference to include a version
            # query string derived from app.js's own mtime. Combined
            # with the immutable Cache-Control on `?v=` URLs below,
            # the browser skips even 304 revalidation on subsequent
            # loads. URL changes automatically when app.js does.
            if rel == "index.html":
                try:
                    app_mtime = (DIST / "app.js").stat().st_mtime
                    tag = f'src="/app.js?v={int(app_mtime)}"'
                    data = data.replace(b'src="/app.js"', tag.encode())
                except Exception:
                    pass
        ...
        # entirely on subsequent loads.
        # KK2 (v2.66.29) — also any URL with `?v=` query string; the
        # bundle-ref rewrite above gives /app.js a versioned URL that
        # changes whenever app.js's mtime does, so it's safe to mark
        # immutable too.
        is_vendor = path.startswith("/vendor/")
        has_versioned = "?v=" in (self.path or "")
        self.send_response(200)
        self.send_header("Content-Type", ct)
        self.send_header("Content-Length", str(len(body)))
        if is_vendor or has_versioned:
